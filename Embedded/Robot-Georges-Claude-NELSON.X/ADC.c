...
void InitADC1(void) {
    ...
    /************************************************************/
    //ADC utilisés : 16(G9)-15(E15)-11(C11)-6(C0)-3(B1)
    ANSELBbits.ANSB1 = 1;
    ANSELCbits.ANSC0 = 1;
    ANSELCbits.ANSC11 = 1;
    ANSELEbits.ANSE15 = 1;
...
void __attribute__((interrupt, no_auto_psv)) _AD1Interrupt(void) {
    ...
    ADCResult[1] = ADC1BUF1; // Read the AN6 input 1 conversion result
...
unsigned int * ADCGetResult(void) {
    return ADCResult; //Retourne les 5 résultats (16 bits) 
}

unsigned char ADCIsConversionFinished(void) {
    return ADCConversionFinishedFlag; //Test si la conversion est finie
}

void ADCClearConversionFinishedFlag(void) {
    ADCConversionFinishedFlag = 0; //Remet le bit de flag de conversion à 0
}